...
- **不是所有函数都能沉**。涉及 `chrome.*` / `document` / `fetch`,或者用了
  ...
  逐个打印跳过原因,不会静默忽略。
- **没有可下沉的函数时,不会以内置示例 wasm 替代**。报告会说明扫描的函数总数,
  以及各函数未能入选的原因;产物中不含 wasm。如此处理是为了避免造成
  "包含 core.wasm 即表示已受保护"的误判。
...
界面支持**中英文切换**(右上角 `中文 / EN`),默认跟随浏览器语言 —— 中文浏览器显示中文、其余显示英文,
选择会记住;也可以用 `http://localhost:4173/?lang=zh`(或 `?lang=en`)直接指定语言。
